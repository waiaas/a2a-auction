...
> 7/30 시점 재계획. 실제 달력 기준 7/30=목, 8/1~8/2=주말, 8/3(월) 23:59 KST 마감. 주말은 작업하지 않기로 확정(아래 트리거 2건에 걸릴 때만 예외). **아래 D-2~D-0 절의 날짜 라벨(8/1 금 등)은 구계획**이며 이 절이 우선한다.

- [ ] **7/30(목)**: 환경 복구(밸리데이터·오케스트레이터·seller 재기동 + 시드) + 리허설 라운드 → **팀 공유**(데모 시연 + 피치덱 초안) + 피드백 수집 시작. 여유 시 피치덱용 화면 캡처
- [ ] **7/31(금)**: 오전 피드백 반영(코드) + **devnet 배포 타임박스 2~3시간**(GCP $300 크레딧 확보·SOL 소요 ~2 실측으로 차단재 해소. 실패 시 localnet 확정 즉시 복귀) + 팀 결정 5건(레포 public 승인 / devnet 성공 시 라이브 URL 여부 / 영상 주체 / 제출 폼 담당 / **결제 스택 요건 운영진 문의 발송 여부** ← 이것만은 7/30 중 결정 필요) → **오후 코드 프리즈** + `verify-e2e.sh` → 저녁 **데모 영상 3분 촬영·편집**(온체인 결제 전 과정 콘티 확인 + `GEMINI_API_KEY` 설정해 라이브 호출로 촬영) + 피치덱 내용 확정 + **레포 public 전환**(승인 즉시)
- [ ] **8/3(월)**: **피치덱 장표 제작 + PDF 변환**(제출 폼 필수: "발표 장표 형태 PDF". 타깃·도입 시나리오 라벨 보강 포함) + 제출 폼(트랙 C) + 최종 `npm run seed` 재실행 + 검증 라운드 1회 → **23:59 전 제출**
- 제출 폼 확인(7/30): 필수 = ① 소개서 PDF(타깃/해결 문제/도입 시나리오+아키텍처) ② repo(재현 코드+README) ③ 영상 3분 이내(실제 온체인 결제 전 과정) / 권장 = 라이브 URL. **원페이저는 요구되지 않음 → 제거**
- 주말 예외 트리거: ① 금요일 저녁까지 영상 미완(편집만 이월 가능) ② 금요일 밤 이후 제출 차단급 피드백 발견(수정 + 재검증, 영상 재촬영 여부 판단)
- 코드 프리즈 이후 도착하는 피드백은 코드 반영 없이 GitHub 이슈 등록으로만 처리
...
- [x] **피치덱 초안** ✅ (7/29, 코어 10장 + 부록 5장. 스펙 **7장** 3분 스크립트는 실증 파트 재료로만 사용, 목차는 문제→해법→실증→BM→로드맵으로 재구성. 발표 자료는 워크스페이스에서 별도 관리, 팀 리뷰 중)
- [ ] 원페이저 (8/3, 피치덱 압축 1장으로 축소 확정)
...
- [x] **C2** 클론 직후 재현 불가 2건 ([#12](https://github.com/waiaas/a2a-auction/pull/12), [#19](https://github.com/waiaas/a2a-auction/pull/19)) — `onchain/deployer.json` 생성·airdrop을 1단계에, `app/web` 빌드를 5단계 필수 선행으로. deployer는 배포뿐 아니라 **시드의 payer**인데 `seed.js`의 airdrop 대상이 아니다.
  - ⚠️ **2차 교차검증에서 추가 발견(#19)**: ① 디렉터리 체인이 3곳(`cd infra`·`cd app`·`cd onchain`) 여전히 상대경로였다. ② `anchor keys sync` 후 **재빌드 안내가 없어** 클론 사용자가 `DeclaredProgramIdMismatch`(0x1004)로 전량 실패한다. `onchain/target/`이 gitignore라 이건 조건부가 아니라 **기본 경로**다.
  - ✅ **실측으로 확인**(새 키페어 + `--reset` 독립 밸리데이터): `solana-keygen new --no-bip39-passphrase`는 프롬프트 없이 통과, **5 SOL 충분**(배포 1.9495 + 시드 0.0138 = 약 1.96 SOL, 2.5배 여유), `dist` 없으면 `:4000`이 실제로 404.
