...
/**
 * 슬라이더 기본값. 첫 진입 화면이 와이어프레임 02(가격 30 · 품질 70, Deep Research 1위)와
 * 같아야 발표 대본이 화면과 어긋나지 않는다. 반반(0.5)이면 최저가가 1위로 잡혀 반대가 된다.
 * 30%는 1-2위 격차 15.1로 뒤집힘 지점(40~45%)에서 넉넉히 떨어져 있고, 시연이 여기서 시작해
 * 55%(격차 17.0)로 미는 동선이라 기본값이 곧 시연 시작점이다.
 */
export const DEFAULT_PRICE_WEIGHT = 0.3;
