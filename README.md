...
> 8/3 제출본(3자 봉인 입찰 경매)과 발표용 고정 라운드는 `/#live` · `/#purchase` 해시에 보존돼 있습니다. 옛 링크를 살리고 [온체인 증거](#온체인-증거)를 재현하는 경로이며, 여는 법은 [실행 방법의 데모 실행 절](#6-데모-실행)에 있습니다.
...
콘티 고정 라운드 **`/#purchase`** 는 공용 데모 지갑으로 고정 요청 3건(5 · 10 · 20 USDC)을 돌려, 같은 정책이 금액마다 어떻게 반응하는지(알림 · 유예 · 승인)를 한 화면에서 보여줍니다.
